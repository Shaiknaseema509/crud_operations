...
function callOrginate()
		{		
			alert('Call Originate initiated');
			 ...
			 var phonenum = "<?php echo $GLOBALS['phone']; ?>";
			 var agentid = "<?php echo $GLOBALS['agent_id']; ?>";
			// var hostip=<?=$GLOBALS["host_ip"];?>;
			 alert(hostip+phonenum+agentid);
			ConVoxClick_to_call_url ="http://"+hostip+"/ConVox3.0/Agent/bridge.php?ACTION=CALL&user="+agentid+"&phone_number="+phonenum; 	
		     alert(ConVoxClick_to_call_url);
			$.post( ConVoxClick_to_call_url, {que:1}, function(return_data)
			{ 		 
			
				var text = return_data;
				//alert(text);
				var myArray = text.split('"CONVOXID":"');
				//alert(myArray[1]);
				var convoxidValue = myArray[1].split('"}');
				$('#convoxidValue').val(convoxidValue[0]);
				$convoxid = convoxidValue;
				alert($convoxid);
			});
		}
// const call = document.getElementById('call')
  ...
  // call.addEventListener('click', () => {
    // alert('You triggered this alert message!', 'success')
 // })
    </script>
</head>
